...
Goal:
  ...
  2) Select CTh alone or add K1/K2 with --features.
  3) Train a diffusion model directly on coefficient tensors
     [number_of_selected_features, (degree+1)^2].
  4) Sample conditionally from demographics such as subject/group/sex/age/days/hemi.
  5) Reconstruct generated coefficients back to vertex-space maps.
...
Feature ablation examples:
  --features CTh
  --features CTh,K1
  --features CTh,K2
  --features CTh,K1,K2

For a fair K1/K2 ablation, use the same seed and set
--aux_channel_mask_prob 0 in every run.
...
def parse_feature_list(s: str) -> List[str]:
    """Parse comma-separated feature names. Valid: CTh,K1,K2."""
    if s is None or str(s).strip() == "":
        return list(FEATURE_NAMES)
    feats = [x.strip() for x in str(s).split(",") if x.strip()]
    if len(feats) == 0:
        return list(FEATURE_NAMES)
    bad = [f for f in feats if f not in FEATURE_TO_INDEX]
    if bad:
        raise ValueError(f"Unknown feature(s): {bad}. Valid features are {FEATURE_NAMES}")
...
def feature_indices(feature_names: Sequence[str]) -> List[int]:
    return [FEATURE_TO_INDEX[str(f)] for f in feature_names]


def parse_channel_loss_weights(s: str, n_channels: int) -> Optional[List[float]]:
    """Parse e.g. '1,0.3,0.3'. Empty means all ones."""
    if s is None or str(s).strip() == "":
        return None
    vals = [float(x.strip()) for x in str(s).split(",") if x.strip()]
    if len(vals) != n_channels:
        raise ValueError(f"--channel_loss_weights expects {n_channels} values, got {len(vals)}: {s}")
    if any(v < 0 for v in vals):
        raise ValueError("--channel_loss_weights values must be non-negative")
    if sum(vals) <= 0:
        raise ValueError("--channel_loss_weights must contain at least one positive value")
    return vals


def expand_selected_coeffs_to_all_features(
    coeffs_selected: np.ndarray,
    selected_features: Sequence[str],
    fill_value: float = 0.0,
) -> np.ndarray:
    """
    Expand selected coeffs [C_sel,L] back to [3,L] so older restore/compare
    scripts that assume CTh,K1,K2 do not crash. Missing channels are filled
    with zeros and should be ignored.
    """
    coeffs_selected = np.asarray(coeffs_selected, dtype=np.float32)
    if coeffs_selected.shape[0] == len(FEATURE_NAMES) and list(selected_features) == list(FEATURE_NAMES):
        return coeffs_selected
    L = coeffs_selected.shape[1]
    full = np.full((len(FEATURE_NAMES), L), fill_value, dtype=np.float32)
    for src_i, feat in enumerate(selected_features):
        full[FEATURE_TO_INDEX[feat]] = coeffs_selected[src_i]
    return full
...
def _prepare_fs_worker_task(task: Tuple[Any, ...]) -> Tuple[str, str, str]:
    """Worker-side FreeSurfer -> SPHARM task. Must stay top-level for multiprocessing."""
    (
        surf_dir,
        hemi,
        out_path,
    ...
    try:
        prepare_one_freesurfer_hemi(
            ...
            out_path=out_path,
            degree=degree,
            sphere_file_template=sphere_file,
        ...
        if strict:
            raise
        return "failed", str(out_path), str(exc)
...
def mode_prepare_fs(args: argparse.Namespace) -> None:
    ...
    hemis = [h.strip() for h in args.hemis.split(",") if h.strip() in ("lh", "rh")]
    ...
    logger.info("prepared task list: tasks=%d workers=%d", len(tasks), workers)

    def _update_counts(status: str, path: str, msg: str) -> None:
        nonlocal n_saved, n_skipped, n_failed
        if status == "saved":
            n_saved += 1
            logger.info("saved [%d/%d]: %s", n_saved + n_skipped + n_failed, len(tasks), path)
        elif status == "skipped":
            n_skipped += 1
            logger.info("skip existing [%d/%d]: %s", n_saved + n_skipped + n_failed, len(tasks), path)
        else:
            n_failed += 1
            logger.warning("failed [%d/%d]: %s %s", n_saved + n_skipped + n_failed, len(tasks), path, msg)

    if workers <= 1:
        for task in tasks:
            status, path, msg = _prepare_fs_worker_task(task)
            _update_counts(status, path, msg)
    else:
        logger.info("parallel prepare start: workers=%d", workers)
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(_prepare_fs_worker_task, task) for task in tasks]
            for fut in as_completed(futures):
                status, path, msg = fut.result()
                _update_counts(status, path, msg)

    logger.info(
        "prepare done: saved=%d skipped=%d failed=%d out_dir=%s",
        n_saved, n_skipped, n_failed, args.out_dir
    )
...
class BrainSPHARMDataset(Dataset):
    def __init__(
        ...
        self.feature_names = parse_feature_list(",".join(feature_names) if feature_names is not None else ",".join(FEATURE_NAMES))
        self.feature_indices = feature_indices(self.feature_names)
        ...
        for p in self.files:
            ...
            x_all = d["coeffs"].astype(np.float32)
            if x_all.ndim != 2 or x_all.shape[1] != self.seq_length:
                raise ValueError(f"Bad coeff shape in {p}: {x_all.shape}, expected [C,{self.seq_length}]")
            if x_all.shape[0] < len(FEATURE_NAMES):
                # Older CTh-only outputs can be reloaded only when compatible.
                if max(self.feature_indices) >= x_all.shape[0]:
                    raise ValueError(
                        f"Cannot select features {self.feature_names} from {p}: coeffs has only {x_all.shape[0]} channel(s)"
                    )
            x = x_all[self.feature_indices]
            if x.shape != (len(self.feature_names), self.seq_length):
                raise ValueError(
                    f"Bad selected coeff shape in {p}: {x.shape}, expected {(len(self.feature_names), self.seq_length)}"
                )
            coeffs.append(x)
            metas.append(
                {
...
class BrainSPHARMResConvNet(nn.Module):
    ...
    x_t contains only the channels selected by --features.
    For auxiliary cross-feature learning, the network also receives:
      known_values: [B,C,L]
      known_mask:   [B,C,L]
    Input to the first conv is concat([x_t, known_values, known_mask])
    and therefore has 3*C channels.
...
class BrainSPHARMDiffusion(nn.Module):
    def __init__(
        ...
        channel_loss_weights: Optional[Sequence[float]] = None,
        feature_names: Optional[Sequence[str]] = None,
        cth_detail_loss_weight: float = 0.0,
        cth_detail_power: float = 1.0,
    ) -> None:
        super().__init__()
        self.model = model
        self.degree = degree
        ...
        self.feature_names = list(feature_names) if feature_names is not None else [
            f"channel_{i}" for i in range(self.feature_channels)
        ]
        if len(self.feature_names) != self.feature_channels:
            raise ValueError(
                f"feature_names length {len(self.feature_names)} "
                f"!= feature_channels {self.feature_channels}"
            )
        self.cth_channel_index = (
            self.feature_names.index("CTh") if "CTh" in self.feature_names else None
        )
        self.cth_detail_loss_weight = float(cth_detail_loss_weight)
        self.cth_detail_power = float(cth_detail_power)
        ...
        loss_w = spharm_loss_weights(degree, low_freq_boost=low_freq_boost)
        if high_freq_boost is not None and float(high_freq_boost) > 0:
            deg = spharm_degree_index(degree).float()
            hf = 1.0 + float(high_freq_boost) * (
                deg / max(float(degree), 1.0)
            ).pow(2.0)
            hf = hf / hf.mean()
            loss_w = loss_w * hf.view(1, 1, -1)
            loss_w = loss_w / loss_w.mean()
        self.register_buffer("loss_weights", loss_w)
        if channel_loss_weights is None:
            cw = torch.ones(1, self.feature_channels, 1, dtype=torch.float32)
        else:
            if len(channel_loss_weights) != self.feature_channels:
                raise ValueError(
                    f"channel_loss_weights length {len(channel_loss_weights)} != feature_channels {self.feature_channels}"
                )
            cw = torch.tensor(channel_loss_weights, dtype=torch.float32).view(1, self.feature_channels, 1)
        cw = cw / torch.clamp(cw.mean(), min=1e-8)
        self.register_buffer("channel_loss_weights", cw)

        deg = spharm_degree_index(degree).float()
        lap = deg * (deg + 1.0)
        lap = lap / max(float(degree * (degree + 1)), 1.0)
        lap = torch.clamp(lap, min=0.0).pow(
            max(float(cth_detail_power), 0.0)
        )
        self.register_buffer("cth_detail_weights", lap.view(1, 1, -1))
    ...
    def make_known_channel_mask(self, x_start: torch.Tensor) -> torch.Tensor:
        ...
        if self.aux_channel_mask_prob <= 0 or C <= 1:
        ...
        for b in range(B):
            if torch.rand((), device=x_start.device) < self.aux_channel_mask_prob:
                ...
                requested = 1 if torch.rand((), device=x_start.device) < 0.5 else 2
                n_known = min(requested, C - 1)
    ...
    def forward(self, x_start: torch.Tensor, cond_vec: torch.Tensor) -> torch.Tensor:
        ...
        v_loss = (v_loss * self.loss_weights * self.channel_loss_weights * loss_channel_weight).mean()
        x0_loss = F.smooth_l1_loss(pred_x0, x_start, reduction="none", beta=0.5)
        x0_loss = (x0_loss * self.loss_weights * self.channel_loss_weights * loss_channel_weight).mean()

        if self.cth_detail_loss_weight > 0 and self.cth_channel_index is not None:
            cth_idx = self.cth_channel_index
            pred_cth_detail = (
                pred_x0[:, cth_idx:cth_idx + 1] * self.cth_detail_weights
            )
            true_cth_detail = (
                x_start[:, cth_idx:cth_idx + 1] * self.cth_detail_weights
            )
            detail_loss = F.smooth_l1_loss(
                pred_cth_detail,
                true_cth_detail,
                beta=0.25,
                reduction="mean",
            ...
            detail_loss = x0_loss.new_tensor(0.0)

        return (
            v_loss
            + self.x0_loss_weight * x0_loss
            + self.cth_detail_loss_weight * detail_loss
        )
...
class TrainConfig:
    ...
    features: str = "CTh,K1,K2"
    channel_loss_weights: str = ""
    val_split: str = "val"
    epochs: int = 1000
    batch_size: int = 8
    lr: float = 2e-4
...
def validate_coeff_mae(
    ...
    total = 0.0
    count = 0
    ch_total: Optional[torch.Tensor] = None
    ch_count: Optional[torch.Tensor] = None
    for batch in val_loader:
        ...
        abs_err = torch.abs(pred_x0 - x)
        total += abs_err.sum().item()
        count += x.numel()
        per_ch_sum = abs_err.sum(dim=(0, 2)).detach().cpu()
        per_ch_count = torch.full_like(per_ch_sum, fill_value=x.shape[0] * x.shape[2])
        if ch_total is None:
            ch_total = per_ch_sum
            ch_count = per_ch_count
        else:
            ch_total += per_ch_sum
            ch_count += per_ch_count
    mae = total / max(count, 1)
    if feature_names is not None and ch_total is not None and ch_count is not None:
        ch_mae = (ch_total / torch.clamp(ch_count, min=1)).numpy().tolist()
        detail = " ".join([f"{name}={val:.8f}" for name, val in zip(feature_names, ch_mae)])
        logger.info("validation normalized coefficient MAE=%.8f | %s", mae, detail)
    else:
...
def save_epoch_samples(
    ...
    for j, idx in enumerate(idxs):
        ...
        coeffs_selected = x_orig[j].astype(np.float32)
        # For CTh-only runs, keep coeffs as [3,L] with dummy K1/K2=0 so existing npz_to_mesh.py still works.
        # The actual generated channels are recorded in selected_feature_names.
        coeffs_for_legacy = expand_selected_coeffs_to_all_features(coeffs_selected, dataset.feature_names)
        np.savez_compressed(
            save_path,
            coeffs=coeffs_for_legacy.astype(np.float32),
            coeffs_selected=coeffs_selected.astype(np.float32),
            feature_names=np.asarray(FEATURE_NAMES),
            selected_feature_names=np.asarray(dataset.feature_names),
...
def mode_train(args: argparse.Namespace) -> None:
    cfg = TrainConfig(
        ...
        channel_loss_weights=args.channel_loss_weights,
        ...
        cth_detail_loss_weight=args.cth_detail_loss_weight,
        cth_detail_power=args.cth_detail_power,
        ...
        seed=args.seed,
        amp=not args.no_amp,
        skip_nan_batches=not args.no_skip_nan_batches,
    )
    ...
    train_feature_names = parse_feature_list(cfg.features)
    channel_loss_weights = parse_channel_loss_weights(cfg.channel_loss_weights, len(train_feature_names))
    logger.info("training features: %s channel_loss_weights=%s", train_feature_names, channel_loss_weights)
    train_dataset = BrainSPHARMDataset(
        cfg.data_dir,
        "train",
        degree=cfg.degree,
    ...
    model = BrainSPHARMResConvNet(
        degree=cfg.degree,
        feature_channels=len(train_dataset.feature_names),
    ...
    diffusion = BrainSPHARMDiffusion(
        ...
        high_freq_boost=cfg.high_freq_boost,
        ...
        cth_detail_loss_weight=cfg.cth_detail_loss_weight,
        cth_detail_power=cfg.cth_detail_power,
    ).to(device)
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        for batch in train_loader:
            ...
            if cfg.skip_nan_batches and not torch.isfinite(loss):
                logger.warning("skip non-finite loss batch at epoch=%d loss=%s", epoch, str(float(loss.detach().cpu()) if loss.numel() == 1 else loss))
                optimizer.zero_grad(set_to_none=True)
                #scaler.update()
                continue
            ...
            if cfg.grad_clip > 0:
                grad_norm = torch.nn.utils.clip_grad_norm_(diffusion.parameters(), cfg.grad_clip)
                if cfg.skip_nan_batches and not torch.isfinite(grad_norm):
                    logger.warning("skip non-finite grad batch at epoch=%d grad_norm=%s", epoch, str(grad_norm))
                    optimizer.zero_grad(set_to_none=True)
                    scaler.update()
                    continue
        ...
        if epoch % cfg.validation_every == 0:
            validate_coeff_mae(diffusion, ema.module, val_loader, feature_names=train_dataset.feature_names)
...
def load_model_from_checkpoint(ckpt_path: str, device: torch.device) -> Tuple[BrainSPHARMDiffusion, BrainSPHARMResConvNet, Dict[str, Any]]:
    ...
    ckpt_feature_names = [str(x) for x in ckpt.get("feature_names", FEATURE_NAMES)]
    feature_channels = len(ckpt_feature_names)
    model = BrainSPHARMResConvNet(
        degree=degree,
    ...
    diffusion = BrainSPHARMDiffusion(
        ...
        high_freq_boost=float(cfg_dict.get("high_freq_boost", 0.0)),
        ...
        cth_detail_loss_weight=float(cfg_dict.get("cth_detail_loss_weight", 0.0)),
        cth_detail_power=float(cfg_dict.get("cth_detail_power", 1.0)),
    ).to(device)
    if "ema_state" in ckpt:
...
def mode_sample(args: argparse.Namespace) -> None:
    ...
    for i in range(args.n):
        ...
        coeffs_selected = x_orig[i].astype(np.float32)
        coeffs_for_legacy = expand_selected_coeffs_to_all_features(coeffs_selected, ckpt_feature_names)
        np.savez_compressed(
            path,
            coeffs=coeffs_for_legacy.astype(np.float32),
            coeffs_selected=coeffs_selected.astype(np.float32),
            feature_names=np.asarray(FEATURE_NAMES),
            selected_feature_names=np.asarray(ckpt_feature_names),
...
def mode_reconstruct(args: argparse.Namespace) -> None:
    ...
    # Prefer selected coefficients when present; otherwise fall back to legacy coeffs.
    if "coeffs_selected" in coeff_npz:
        coeffs = coeff_npz["coeffs_selected"].astype(np.float32)
        feature_names = [str(x) for x in coeff_npz.get("selected_feature_names", ["CTh"])]
    else:
        coeffs = coeff_npz["coeffs"].astype(np.float32)
        feature_names = [str(x) for x in coeff_npz.get("feature_names", FEATURE_NAMES)]
        if len(feature_names) != coeffs.shape[0]:
            feature_names = list(FEATURE_NAMES[: coeffs.shape[0]])
    ...
    for i, name in enumerate(feature_names):
        save_dict[name] = maps[i].astype(np.float32)
    ...
    if args.write_morph_dir:
        ...
        for i, name in enumerate(feature_names):
            if name == "CTh":
                fname = f"{args.hemi}.generated.thickness"
            elif name == "K1":
                fname = f"{args.hemi}.generated.smoothwm.K1.crv"
            elif name == "K2":
                fname = f"{args.hemi}.generated.smoothwm.K2.crv"
            else:
                fname = f"{args.hemi}.generated.{name}.mgh"
            fsio.write_morph_data(str(out_dir / fname), maps[i].astype(np.float32))
...
def add_prepare_fs_parser(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--prepare_workers", type=int, default=1, help="Number of parallel FreeSurfer hemi preprocessing workers")
...
def add_train_parser(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        ...
        default="CTh,K1,K2",
        help=(
            "channels to train. Ablations: CTh | CTh,K1 | "
            "CTh,K2 | CTh,K1,K2"
        ),
    )
    p.add_argument(
        ...
        default="",
        help=(
            "optional weights matching --features. For CTh,K1,K2, "
            "use '1,0.2,0.2' to make CTh dominant"
        ),
    )
    ...
    p.add_argument("--high_freq_boost", type=float, default=0.0)
    p.add_argument("--x0_loss_weight", type=float, default=0.25)
    p.add_argument(
        "--cth_detail_loss_weight",
        type=float,
        default=0.0,
        help="extra high-frequency CTh reconstruction loss",
    )
    p.add_argument("--cth_detail_power", type=float, default=1.0)
    ...
    p.add_argument("--no_skip_nan_batches", action="store_true", help="Disable skipping non-finite loss/grad batches")
